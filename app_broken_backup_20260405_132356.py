...
def get_applications():
    rows = safe_rows("loan_applications", "created_at", True)
    if rows:
...
def get_application_by_id(app_id):
    row = safe_find_by_id("loan_applications", app_id)
    if row:
...
def customer_secure_apply():
    ...
    try:
        ...
        loan_amount = to_int(form.get("loan_amount"), 0)
        ...
        monthly_income = to_int(form.get("monthly_income"), 0)
        other_income = to_int(form.get("other_income"), 0)

        reference = "KATRA-" + datetime.now(UTC).strftime("%Y%m%d") + "-" + uuid.uuid4().hex[:6].upper()
        ...
        payload = {
            ...
            "geo_lat": to_str(form.get("geo_lat")),
            "geo_lng": to_str(form.get("geo_lng")),
            "geo_accuracy": to_str(form.get("geo_accuracy")),
            "geo_timestamp": to_str(form.get("geo_timestamp")),
...
def customer_dashboard():
    ...
    if (not profile) and applications:
        latest = applications[0]
        profile = {
            "full_name": latest.get("full_name") or "",
            "phone": latest.get("phone") or latest.get("contact_number") or "",
            "email": latest.get("email") or email,
            "physical_address": latest.get("physical_address") or "",
            "town_city": latest.get("town_city") or "",
    ...
    if applications:
        latest = applications[0]
        current_status = str(latest.get("status") or "SUBMITTED").upper()
        approved_amount = int(float(latest.get("amount") or 0))

    paid_amount = 0
    try:
        paid_amount = sum(
            int(float(x.get("amount") or x.get("paid_amount") or 0))
            for x in loan_payments
        )
    except Exception:
        paid_amount = 0

    balance_amount = int(max(approved_amount - paid_amount, 0))
...
def customer_support_new():
    try:
        ...
        supabase_admin.table("support_tickets").insert(payload).execute()
        return redirect(url_for("customer_dashboard"))
    except Exception as e:
...
def customer_secure_apply():
    ...
    if (not profile) and applications:
        ...
        profile = {
            "full_name": latest_app.get("full_name") or "",
            "phone": latest_app.get("phone") or "",
            "email": latest_app.get("email") or email,
            "physical_address": latest_app.get("physical_address") or "",
            "town_city": latest_app.get("town_city") or "",
            "region": latest_app.get("region") or ""
        }
...
if (not profile) and applications:
    ...
    profile = {
        "full_name": latest_app.get("full_name") or "",
        "phone": latest_app.get("phone") or latest_app.get("contact_number") or "",
        "email": latest_app.get("email") or email,
        "physical_address": latest_app.get("physical_address") or "",
        "town_city": latest_app.get("town_city") or "",
        "region": latest_app.get("region") or ""
    ...
    balance_amount = int(max(to_float(approved_amount, 0) - to_float(paid_amount, 0), 0))
...
def customer_dashboard():
    ...
    # fallback profile from application
    if (not profile) and applications:
